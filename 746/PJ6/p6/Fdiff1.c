...
*/
  ...
  cl_mem device_u0 = clCreateBuffer(context, CL_MEM_READ_WRITE, memory_size, NULL, &ret);
  printError(ret);
  cl_mem device_u1 = clCreateBuffer(context, CL_MEM_READ_WRITE, memory_size, NULL, &ret); 
  printError(ret);
  cl_mem device_width = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int), NULL, &ret);
  printError(ret);
...
//  double t0 = tv.tv_sec*1e6 + tv.tv_usec;
  ...
  ret = clEnqueueWriteBuffer(command_queue, device_u0, CL_TRUE, 0, memory_size, u0, 0, NULL, NULL);
  printError(ret);
  ret = clEnqueueWriteBuffer(command_queue, device_u1, CL_TRUE, 0, memory_size, u1, 0, NULL, NULL);
    printError(ret);
  ret = clEnqueueWriteBuffer(command_queue, device_width, CL_TRUE, 0, sizeof(int), &width, 0, NULL, NULL);
  printError(ret);
  ...
  // Create the OpenCL kernel
	...
	size_t global_size = (((width * width) / (TILE_SIZE * TILE_SIZE)) + 1) * TILE_SIZE * TILE_SIZE;
